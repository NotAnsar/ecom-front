...
export const cartStorageSlice = createSlice({
	...
	reducers: {
		...
		addWish: (state, action) => {
			const check = state.wish.find((w) => w._id === action.payload._id);
		...
		removeWish: (state, action) => {
			state.wish = state.wish.filter((w) => w._id !== action.payload._id);
		...
		addCart: (state, action) => {
			...
			if (!existingItem) {
				...
				state.cart.forEach((cart) => {
					if (action.payload._id === cart._id) {
		...
		addItem: (state, action) => {
			state.cart.forEach((cart) => {
				if (action.payload.id === cart._id) cart.qte++;
			});
